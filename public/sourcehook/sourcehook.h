...
enum SH_META_RES
{
	SH_MRES_IGNORED=0,		// plugin didn't take any action
	SH_MRES_HANDLED,		// plugin did something, but real function should still be called
	SH_MRES_OVERRIDE,		// call real function, but use my return value
	SH_MRES_SUPERCEDE		// skip real function; use my return value
};
	...
	*/
	...
	public:
		...
		*/
		...
		virtual void SetRes(SH_META_RES res) = 0;				//!< Sets the meta result
		virtual SH_META_RES GetPrevRes() = 0;					//!< Gets the meta result of the
															//!<  previously calledhandler
		virtual SH_META_RES GetStatus() = 0;					//!< Gets the highest meta result
		...
		virtual void SetCurResPtr(SH_META_RES *mres) = 0;			//!< Sets pointer to the current meta result
		virtual void SetPrevResPtr(SH_META_RES *mres) = 0;			//!< Sets pointer to previous meta result
		virtual void SetStatusPtr(SH_META_RES *mres) = 0;			//!< Sets pointer to the status variable
		...
		*/
		virtual void *SetupHookLoop(SH_META_RES *statusPtr, SH_META_RES *prevResPtr, SH_META_RES *curResPtr,
...
/************************************************************************/
#define SH_META_RESULT_STATUS					SH_GLOB_SHPTR->GetStatus()
#define SH_META_RESULT_PREVIOUS					SH_GLOB_SHPTR->GetPrevRes()
#define SH_META_RESULT_ORIG_RET(type)			*SourceHook::MacroRefHelpers<type>::GetOrigRet(SH_GLOB_SHPTR)
#define SH_META_RESULT_OVERRIDE_RET(type)		*SourceHook::MacroRefHelpers<type>::GetOverrideRet(SH_GLOB_SHPTR)
#define SH_META_IFACEPTR(type)					reinterpret_cast<type*>(SH_GLOB_SHPTR->GetIfacePtr())

#define SH_SET_META_RESULT(result)				SH_GLOB_SHPTR->SetRes(result)
#define SH_RETURN_META(result)					do { SH_SET_META_RESULT(result); return; } while(0)
#define SH_RETURN_META_VALUE(result, value)	do { SH_SET_META_RESULT(result); return (value); } while(0)
...
#define RETURN_META_NOREF(result, rettype)	do { SH_SET_META_RESULT(result); return reinterpret_cast<rettype>(*SH_GLOB_SHPTR); } while(0)
...
#define RETURN_META_VALUE_NEWPARAMS(result, value, memfuncptr, newparams) \
	do { \
		...
		SH_RETURN_META_VALUE(SH_MRES_SUPERCEDE, \
			(SourceHook::RecallGetIface(SH_GLOB_SHPTR, memfuncptr)->*(memfuncptr)) newparams); \
	} while (0)
...
#define RETURN_META_VALUE_MNEWPARAMS(result, value, hookname, newparams) \
	do { \
		...
		SOUREHOOK__MNEWPARAMS_PREPAREMFP(hookname); \
		SH_RETURN_META_VALUE(SH_MRES_SUPERCEDE, (thisptr->*(u.mfp)) newparams); \
	} while (0)
...
#define SH_SETUPCALLS(rettype, paramtypes, params) \
	...
	SH_META_RES status = SH_MRES_IGNORED; \
	SH_META_RES prev_res; \
	SH_META_RES cur_res; \
...
#define SH_RETURN() \
	...
	return status >= SH_MRES_OVERRIDE ? *pOverrideRet : orig_ret;
