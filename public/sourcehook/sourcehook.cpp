...
#undef TMP_CHECK_LIST
	...
	void CSourceHookImpl::HookLoopEnd()
	{
		...
		if (m_HLIStack.size() > 1 && m_HLIStack.second().recall == HookLoopInfo::Recall_Post2)
			*m_HLIStack.front().pStatus = SH_MRES_SUPERCEDE;			// THAT'LL TEACH THEM!
		m_HLIStack.pop();
	}

	void CSourceHookImpl::SetRes(SH_META_RES res)
	{
		*m_HLIStack.front().pCurRes = res;
	}

	SH_META_RES CSourceHookImpl::GetPrevRes()
	{
		return *m_HLIStack.front().pPrevRes;
	}

	SH_META_RES CSourceHookImpl::GetStatus()
	{
	...
	const void *CSourceHookImpl::GetOverrideRet()
	{
		...
		return (*m_HLIStack.front().pStatus < SH_MRES_OVERRIDE) ?
	...
	void CSourceHookImpl::SetCurResPtr(SH_META_RES *mres)
	{
		m_HLIStack.front().pCurRes = mres;
	}

	void CSourceHookImpl::SetPrevResPtr(SH_META_RES *mres)
	{
	...
	void CSourceHookImpl::SetStatusPtr(SH_META_RES *mres)
	{
	...
	void *CSourceHookImpl::SetupHookLoop(SH_META_RES *statusPtr, SH_META_RES *prevResPtr, SH_META_RES *curResPtr,
		void **ifacePtrPtr, const void *origRetPtr, void *overrideRetPtr)
	{
		...
		if (m_HLIStack.size() > 1 && m_HLIStack.second().recall)
		{
			...
			*prevResPtr = *other.pStatus;
			...
			if (*statusPtr < SH_MRES_OVERRIDE && other.recall == HookLoopInfo::Recall_Post1)
	...
	bool CSourceHookImpl::ShouldContinue()
	{
		...
		if (m_HLIStack.size() > 1)
		{
			if (m_HLIStack.second().recall == HookLoopInfo::Recall_Post1)
			{
				...
				*m_HLIStack.front().pStatus = SH_MRES_SUPERCEDE;
